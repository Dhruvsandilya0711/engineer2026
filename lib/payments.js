...
/**
 * Fees for many events in one go, as { eventName: rupees }, containing only
 * the priced ones. The register form needs this before anything is submitted:
 * a price that first appears after you press the button is a surprise, and
 * surprises about money are the bad kind.
 *
 * Returns {} when payments are off, so every caller's "is this free?" test is
 * the same test whether or not Razorpay is configured.
 */
export async function feeMap(eventNames) {
  const out = {};
  if (!razorpay || !Array.isArray(eventNames) || !eventNames.length) return out;

  const priced = (name, fee) => {
    const n = Number(fee);
  ...
  if (mode === 'mongo' && EVENT_FEE) {
    try {
      const rows = await EVENT_FEE.find({ eventName: { $in: eventNames } }).lean();
      rows.forEach(r => priced(r.eventName, r.eventFee));
    } catch {
      // Same rule as feeFor: a database hiccup must not quietly make a paid
      // event look free. An empty map would do exactly that, so say so.
      throw new Error('fee lookup failed');
    }
    return out;
  }
  ...
  readFileFees().forEach(r => { if (wanted.has(r.eventName)) priced(r.eventName, r.eventFee); });
  return out;
}
